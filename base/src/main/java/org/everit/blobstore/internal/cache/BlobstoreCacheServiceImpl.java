...
import org.everit.util.core.filter.IntervalRelation;
...
public class BlobstoreCacheServiceImpl implements BlobstoreCacheService {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(BlobstoreCacheServiceImpl.class);
	...
	public BlobstoreCacheServiceImpl(
	...
	public List<CachedBlobPart> getBlobParts(final long blobId,
		...
		while (iterator.hasNext() && !collectingFinished) {
			...
			if (fragment != null
					&& fragment.getStartPosition() + fragmentSize >= startPosition) {
				long prevFragmentPartAbsEndPos = 0;
				for (FragmentPart part : fragment.getFragmentParts()) {
					long fragmentPartAbsStartPos = fragment.getStartPosition()
							+ part.getStartPositionInFragment();
					long fragmentPartAbsEndPos = fragmentPartAbsStartPos
							+ part.getData().length;
					...
					// we are interested in
						...
						byte[] partData;
						if (copyFrom != 0
								|| copyLength != part.getData().length) {
							partData = new byte[copyLength];
							System.arraycopy(part.getData(), copyFrom,
									partData, 0, copyLength);
						} else {
							partData = part.getData();
						}
						rval.add(new CachedBlobPart(blobId,
								fragmentPartAbsStartPos + copyFrom, partData));
						actualLength = candidateNewLength;
						if (actualLength >= maxLength) {
							collectingFinished = true;
						} else {
							prevFragmentPartAbsEndPos = fragment
									.getStartPosition()
									+ part.getEndPositionInFragment();
						}
					}
				}
			}
	...
	void mergeInto(final long relStartPos, final FragmentByteArray blobPart,
			final FragmentPart part) {
		long relPosInFragment = relStartPos - part.getStartPositionInFragment();
		blobPart.copyTo(0, part.getData(), (int) relPosInFragment,
				blobPart.length());
	}
	...
	private void partialUpdateBlobFragment(final long relStartPos,
		...
		Interval<Long> newInterval = new Interval<Long>(relStartPos, relEndPos);
		...
		if (fragmentParts.size() > 0) {
			...
			while (iterator.hasNext() && !merged) {
				part = iterator.next();
				Interval<Long> existingInterval = part.asInterval();
				IntervalRelation relation = newInterval
						.getRelationTo(existingInterval);
				// the new interval has nothing to do with this existing
				// interval
				if (!relation.isDistinct()) {
					if (relation == IntervalRelation.CONTAINING
							|| relation == IntervalRelation.IDENTICAL) {
						mergeInto(relStartPos, fragmentBytes, part);
						// the entire blobPart has been copied into the
						// containing FragmentPart
						// nothing else to do
						merged = true;
					} else if (relation == IntervalRelation.BEFORE_OVERLAPPING) {
						beforeOverlapping = part;
					} else if (relation == IntervalRelation.AFTER_OVERLAPPING) {
						afterOverlapping = part;
					} else {
						assert (relation == IntervalRelation.CONTAINED);
						innerParts.add(part);
					}
				}
			}
			...
			FragmentPart newPart = new FragmentPart(relStartPos,
					fragmentBytes.asRawArray());
			fragment.insertFragmentPart(newPart);
		}
	...
	/**
	 * Used during the blob part storing process to merge the
	 * {@link FragmentPart}s which already exist in the cache and overlap (but
	 * aren't contained by) the new blob part to be stored. The
	 * <code>beforeOverlapping</code> fragment part, the new fragment part and
	 * the <code>afterOverlapping</code> fragment part must be in the same
	 * {@link Fragment} specified by <code>ownerFragment</code>, cross-fragment
	 * data storing is not taken into account by this method.
	 *
	 * Merges the before-overlapping {@link FragmentPart}, after-overlapping
	 * {@link FragmentPart} (if they exist) and the new blob data into a new
	 * array.
	 *
	 * If <code>beforeOverlapping</code> and <code>afterOverlapping</code> are
	 * both null then the method will return <code>blobPart</code> otherwise it
	 * performs the merge.
	 *
	 * Example: if <code>beforeOverlapping</code> and
	 * <code>afterOverlapping</code> look this way (given by the start and end
	 * position relatively in the <code>ownerFragment</code>):
	 * <ul>
	 * <li><code>beforeFragment</code>: [5, 20]</li>
	 * <li><code>afterFragment</code>: [80, 95]</li>
	 * </ul>
	 * and <code>startPosition</code> = 10, and <code>blobPart.length</code> is
	 * 80, then the following steps will be performed by this method:
	 * <ul>
	 * <li>creates a new <code>byte</code> array with size 95 - 5 = 90, this
	 * array will be returned</li>
	 * <li>copies the first 5 bytes of {@link FragmentPart#getData()
	 * beforeOverlapping.getData()} to the beginning of the result</li>
	 * <li>copies <code>blobPart</code> to the result array, to position 5 (at
	 * this point the first 85 bytes of the result will be loaded)</li>
	 * <li>copies the last 5 bytes of {@link FragmentPart#getData()
	 * afterOverlapping.getData()} to the result</li>
	 * <li>{@link LinkedList#remove() removes} both
	 * <code>beforeOverlapping</code> and <code>afterOverlapping</code> from the
	 * {@link Fragment#getFragmentParts() the FragmentParts of ownerFragment}.</li>
	...
	private FragmentPart processOverlappings(
		...
		if (beforeIsNull && afterIsNull) {
			return new FragmentPart(startPositionInFragment,
					blobPart.asRawArray());
		}
		...
		if (!beforeIsNull) {
			...
			beforeRemaining = (int) (startPositionInFragment - beforeOverlapping
					.getStartPositionInFragment());
		...
		if (!beforeIsNull) {
			System.arraycopy(beforeFragmentOldData, 0, newData, 0,
					beforeRemaining);
		...
		if (!afterIsNull) {
			System.arraycopy(afterFragmentOldData, afterCut, newData,
					loadedUpTo, afterRemaining);
	...
	public void removePartsByBlobId(final long blobId) {
		for (CacheKey key : keyCache.get(blobId)) {
			if (cache.remove(key) == null) {
				LOGGER.warn(
						"inconsistent cache state: {} exists in keyCache but not found in fragment cache",
						key);
			}
		}
		keyCache.remove(blobId);
	...
	public void storeBlobPart(final long blobId, final long startPosition,
		...
		FragmentByteArray[] bytesByFragment = new FragmentableBlobPart(
				blobPart, (int) fragmentSize, (int) relStartPos).getFragments();
		...
		if (bytesByFragment.length > 1) {
			fragment = getFragment(blobId, fragmentStartPosition);
			partialUpdateBlobFragment(0,
					bytesByFragment[bytesByFragment.length - 1], fragment);
		}
	}
	...
	private void updateBlobFragment(final FragmentByteArray fragmentByteArray,
		...
		FragmentPart newPart = new FragmentPart(0,
				fragmentByteArray.asRawArray());
